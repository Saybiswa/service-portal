...
router.post("/customer", async (req, res) => {
  try {
    ...
    if (!customer_name || !phone1) {
      return res.status(400).json({ error: "Customer name and phone1 are required" });
    }

    const result = await pool.query(
      `INSERT INTO customers(
        customer_name, phone1, phone2, email, state, city, locality, address,
        product, product_type, model_number, serial_number, warranty_status, svc_type, complaint_issue
